<?php
/**
 * Single Image Content Partial
 *
 * Expected args:
 * - post_id: Attachment post ID.
 * - title: Optional image title override.
 *
...
<article class="rounded-lg bg-base-100 p-4">
  <div class="grid grid-flow-row gap-2">
    ...
    <div class="max-w-none text-sm text-base-content/80 [&_p]:py-2 [&_img]:mx-auto [&_img]:cursor-pointer [&_img]:rounded-lg [&_img]:shadow-md" id="content">
      ...
      <?php endif; ?>
      <img src="<?php echo esc_url((string) wp_get_attachment_url((int) ($args['post_id'] ?? get_the_ID()))); ?>" alt="<?php echo esc_attr(get_the_title((int) ($args['post_id'] ?? get_the_ID()))); ?>" class="h-auto w-full rounded-lg shadow-md">
    </div>
